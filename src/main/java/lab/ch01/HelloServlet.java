...
public class HelloServlet extends HttpServlet {
	...
	throws ServletException, IOException{
		...
		log.info("doPut"+req.getParameter("mem_id")+", "+req.getParameter("mem_pw"));
	}
	// 삭제할 때 - DELETTE FROM member WHERE mem_id='kiwi'
	public void doDelete(HttpServletRequest req, HttpServletResponse res)
	throws ServletException, IOException{
		System.out.println("doDelete호출");
		log.info("doDelete"+req.getParameter("mem_id")+", "+req.getParameter("mem_pw"));
	}	
	// 입력 또는 보안상 중요한 요청일 때 -> 노출이 안되니까
	//INSERT INTO MEMBER VALUES('kiwi','123','키위',,,)
	//SELECT mem_name FROM member
	//WHERE mem_id='kiwi' AND mem_pw='123'
	//개발 시에는 get방식으로 단위 테스트를 활용해서 마무리되면 post방식으로 변경해서 Commit 함.
	public void doPost(HttpServletRequest req, HttpServletResponse res)
	throws ServletException, IOException{
		System.out.println("doPost호출");
		log.info("doPost"+req.getParameter("mem_id")+", "+req.getParameter("mem_pw"));
	}
	//서블릿의 doGet메서드는 Restful API가 제공하는 get 메서드와 같다.
	//파라미터 자리는 변수를 선언하는 자리 -> null
	//주의: 사용자가 화면에 입력한 값이(URL을 통해서 노출이 되므로 알고 사용해야 함!)
	public void doGet(HttpServletRequest req, HttpServletResponse res)
	throws ServletException, IOException
	{
		log.info("doGet" + req.getParameter("mem_id")+", "+req.getParameter("mem_pw"));
